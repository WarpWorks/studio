...
class Entity extends Base {
    ...
    canBeInstantiated() {
        ...
        if (parrentAggs && parrentAggs.length) {
            return true;
        }
...
<<<<<<< HEAD
        ...
        const a = this.relationships
            ...
            .map((relationship) => relationship);
=======
        const a = this.relationships.filter((relationship) => !relationship.isAggregation);
>>>>>>> origin/master
        ...
        */
        var a = [];
        for (var i in this.relationships) if (!this.relationships[i].isAggregation) a.push(this.relationships[i]);
        if (!ignoreIngeritedAssociations && this.hasParentClass())
