// ***************************************************************************************************** //
// Web API for Domain: HeadStart
// Generated by HeadStart, version: v0.1
// Generated on: 2017-02-02 21:47:36
// ***************************************************************************************************** //
...
get_HeadStart = function  () {
    if (!_HeadStart)
        _HeadStart = new HeadStart (null, null, "Root", null);
    return _HeadStart;
}

get_Domain_fromJSON = function (json) {
    var domain = get_HeadStart().addNew_Domain(json.name, json.desc, json.id);
...
// Constructor
function HeadStart (parent, id, name, desc) {
    // Initialize base class
    
    Base.call(this, "HeadStart", parent, id, name, desc);
...
// Inheritance
HeadStart.prototype = Object.create(Base.prototype);
HeadStart.prototype.constructor = HeadStart;
...
//
// Manage Aggregations:
HeadStart.prototype.addNew_Domain = function (name, desc, existingID) {
    var id = existingID ? existingID : 1;// 1 == ID for new Domains
    var new_myDomains = new Domain (this, id, name, desc);
    new_myDomains.type = "Domain";
    this.myDomains.push (new_myDomains);
    return new_myDomains;
}
HeadStart.prototype.remove_Domain = function (id) {
    var idx = -1;
    this.myDomains.forEach (function (elem, i) {
        if (elem.compareToMyID(id)) idx = i;
    });
    if(idx != -1)
        this.myDomains.splice(idx, 1);
    else
        throw "Element not found: "+id;
}

// Misc utility functions
HeadStart.prototype.getAllElements = function (includeSelf)
{
    var r = new Array ();
    if (includeSelf) r = r.concat(this);
    for (i in this.myDomains) r = r.concat(this.myDomains[i].getAllElements(true));
    return r;
}

HeadStart.prototype.toJSON = function () {
    // Get JSON for aggregated entities:
    var json_myDomains = [];
    for (i in this.myDomains) json_myDomains.push(this.myDomains[i].toJSON());
    ...
    return {
        ...
        type: this.type,
        id: this.idToJSON(),
        myDomains: json_myDomains
    }
}

HeadStart.prototype.fromJSON = function (json, parent) {
    // Base attibutes:
    this.parent = parent;
    this.name = json.name;
    this.desc = json.desc;
    ...
    for (var i in json.myDomains) {
        var jsElem = json.myDomains[i];
...
BasicProperty.prototype.enumDef_propertyType = ["string", "number", "boolean", "date"];
