...
Entity.prototype.getAggregations = function (sort) {
    ...
    this.relationships.forEach (function (relation) {
        if (relation.isAggregation) res.push(relation);
    });
    if (sort)
...
Domain.prototype.sortElements = function (entities) {
    return entities.sort(function(a,b) {
        if (a.isRootInstance && a.isRootInstance===true) return -1;
        if (b.isRootInstance && b.isRootInstance===true) return 1;
...
Domain.prototype.getRoot = function () {
    ...
    if (root.length != 1) throw "Domain must have exactly one rootEntity, not "+root.length;
    return root[0];
}
