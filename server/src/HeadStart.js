...
// HACK! This would not be necessary if the file was '.json'.
if (!require.extensions['.jsn']) {
    require.extensions['.jsn'] = require.extensions['.json'];
}
...
class HeadStart {
    ...
    getDomainByName(domainName) {
        try {
            const domainFilePath = path.resolve(path.join(this.getDir('domains'), `${domainName}.jsn`));
            const domainJson = require(domainFilePath);
            const domain = this.createDomainFromJSON(domainJson);
            Promise.resolve(domain);
        } catch (e) {
            Promise.reject(new HeadStartError("Failed load domain", e));
        }
    }
    ...
    */
    ...
    createBeginTag(tagName, conditional) {
        if (conditional) {
            return '{{' + tagName + '?}}';
        } else {
            return "{{" + tagName + "*}}";
    ...
    createEndTag(tagName, conditional) {
        if (conditional) {
            return '{{/' + tagName + '?}}';
        } else {
            return "{{/" + tagName + "}}";
