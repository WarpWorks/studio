...
PageView.prototype.setAsDefault = function () {
    for (var i in this.parent.pageViews) { this.parent.pageViews[i].isDefault = false; }
    this.isDefault = true;
};
...
PageView.prototype.getAllElements = function (includeSelf) {
    ...
    for (var i in this.panels) r = r.concat(this.panels[i].getAllElements(true));
    return r;
};

PageView.prototype.toString = function () {
    var s = this.name + " [";
    for (var i in this.panels) s += this.panels[i].toString();
    return s + "]; ";
};

PageView.prototype.toJSON = function () {
    var p = [];
    for (var i in this.panels) p.push(this.panels[i].toJSON());
