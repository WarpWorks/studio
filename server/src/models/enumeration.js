...
Enumeration.prototype.getTestData = function () {
    if (this.literals && this.literals.length > 0) { return this.literals[Math.floor(Math.random() * this.literals.length)].name; } else { return "Undefined"; }
};
...
Enumeration.prototype.toJSON = function () {
    var l = [];
    for (var i in this.literals) l.push(this.literals[i].toJSON());
